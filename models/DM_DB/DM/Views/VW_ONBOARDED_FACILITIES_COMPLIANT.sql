...
inpatient_clinics_w_provisioned_users as (
    with
    //create a cte of all clinics that meet inpatient criteria
    inpatient_clinics as (select * from dm_table_data_clinic as c
    ...
    // generate list of clinics that have user's provisioned
    user_clinics as (select users.hq_user_id, clinics.case_id as clinic_case_id 
    ...
    //pull all clinic case properties from inpatient clinics that have at least one user
    ...
    join dm_table_data_unit as unit
        on unit_ts.case_id = unit.case_id
    join inpatient_clinics_w_provisioned_users as c
