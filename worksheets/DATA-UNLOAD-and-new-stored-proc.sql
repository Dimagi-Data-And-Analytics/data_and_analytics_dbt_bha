...
use schema CO_CARE_COORDINATION_COMMCARE_TEST;
...
-- create task
create or replace task DATALAKE_DEV.CO_CARE_COORDINATION_COMMCARE_TEST.S3_UNLOAD_TASK
	schedule='USING CRON 01 05 * * * America/New_York'
	error_integration=SNS_INT_OBJ
	as DECLARE 
    task_result string default null;
    task_exception EXCEPTION (-20003, 'Task had an error');
BEGIN
    Call metadata.procedures_dev.sp_data_unload('CLIENT_PAYLOAD_UPDATE', 'task_call_sp_data_unload', 'co-carecoordination-test', 'DATALAKE_DEV', 'CO_CARE_COORDINATION_COMMCARE_TEST', 'DM_CO_CARE_COORD_TEST', 'UNLOAD_SF_TO_S3_AWS|', null) into :task_result;
    IF (task_result ilike '%error%') THEN 
        RAISE task_exception;
    END IF;
END;

ALTER TASK DATALAKE_DEV.CO_CARE_COORDINATION_COMMCARE_TEST.S3_UNLOAD_TASK RESUME;
EXECUTE TASK DATALAKE_DEV.CO_CARE_COORDINATION_COMMCARE_TEST.S3_UNLOAD_TASK;
-- end create task
